...
//工具栏
    ...
    toolButton->setText("我的信息");
    toolButton->setToolTip("点击显示个人信息");

    // 添加到工具栏
    toolBar->addWidget(toolButton);

    // 连接信号槽
    connect(toolButton, &QToolButton::clicked, this, &QWMainWind::showAboutInfo);
...
void QWMainWind::on_pushButton_clicked()
{
    ...
    information原型:
        ...
        第二个参数是标题
        第三个参数是内容
        第四个参数是窗口里面要多少个按钮（不填时默认为OK按钮）
        第五个参数指定按下键盘Enter时使用的按钮。（不填时默认为NoButton，此时QMessageBox会自动选择合适的默认值。）
    */
...
void QWMainWind::showAboutInfo()
{
    ...
    QMessageBox::information(this, "我的信息", info);
}
